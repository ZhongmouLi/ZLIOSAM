...
def save_dynamic_tf(bag, kitti, kitti_type, initial_time):
    ...
    if kitti_type.find("raw") != -1:
        for timestamp, oxts in zip(kitti.timestamps, kitti.oxts):
            ...
            tf_oxts_transform.child_frame_id = 'lio_base_link'
...
if __name__ == "__main__":
    ...
    if args.kitti_type.find("raw") != -1:
        ...
        try:
            ...
            T_lio_base_link_to_imu = np.eye(4, 4)
            T_lio_base_link_to_imu[0:3, 3] = [-2.71/2.0-0.05, 0.32, 0.93]

            # tf_static
            transforms = [
                ('lio_base_link', imu_frame_id, T_lio_base_link_to_imu),
